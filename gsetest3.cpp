...
unsigned short int hkp[16],highvoltage,frameno,oldframe,subcom,*bufptr;
		...
		for(;;)
		{
			if( newdisplay == 1)
			{
				pthread_mutex_lock( &mymutex); /* wait on readgse */
					bufptr = buffer;
					frameno = *(bufptr+5);
//					if (frameno != oldframe + 4) printf("Frame mismatch %x %x \n",frameno,oldframe);
					oldframe = frameno; 
					highvoltage = *(bufptr + 13);
					for(i = 0; i < 4; i++)
					{
						frameno = *(bufptr+5);
						subcom = frameno & 0x3;
				...
				if((displayonce != 0) ||(displaycont != 0))
				{
					...
					if(showhkp != 0)
					{
						...
						printf("T8 = %6.1f, T9 = %6.1f, T10 = %6.1f, T11 = %6.1f                        \n",T8,T9,T10,T11);
						printf("HV = 0x%04x %3dV ",highvoltage,highvoltage/128);
						if(highvoltage & 0x1)
						{
							printf("^\r\033[7A");							
						}
						else if(highvoltage & 0x2)
						{
							printf("V\r\033[7A");
						}
						else printf("-\r\033[7A");
					}
					fflush(stdout);
				}
				newdisplay = 0;
