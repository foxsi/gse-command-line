...
int fsercmd,ttyout,devicefile,actelvalue,cmdvalue;
unsigned char cmd[4];
...
//      if((S_IFREG & mystat.st_mode) == 0) ttyout = 1;
        ...
        if( cmdvalue > 255)
        {
			printf (" Command greater than 255, too large %d \n",cmdvalue);
			exit(-1);
		}
		printf("Commanding ACTEL %u to %u  , %x \n",actelvalue,cmdvalue,cmdvalue);
		cmd[0] = 0xc0;
		cmd[1] = 0;
		cmd[1] |= actelvalue;
